(function (root) {
  ...
  const presets = {
    financing: { id: "financing", name: "Financing dominates", spot: 100, rate: 0.05, dividendYield: 0.01, tenor: 2, strike: 100, volatility: 0.2, optionBudget: 10 },
    dividends: { id: "dividends", name: "Dividends dominate", spot: 100, rate: 0.015, dividendYield: 0.045, tenor: 2, strike: 100, volatility: 0.2, optionBudget: 10 },
    neutral: { id: "neutral", name: "Carry neutral", spot: 100, rate: 0.03, dividendYield: 0.03, tenor: 3, strike: 100, volatility: 0.2, optionBudget: 10 },
    longDated: { id: "longDated", name: "Long-dated carry", spot: 100, rate: 0.04, dividendYield: 0.015, tenor: 5, strike: 110, volatility: 0.25, optionBudget: 10 }
  };
  ...
  function optionFromForward(forward, strike, tenor, rate, volatility) {
    if (tenor <= 0 || volatility <= 0) {
      const discountFactor = Math.exp(-rate * Math.max(0, tenor));
      return { call: discountFactor * Math.max(forward - strike, 0), put: discountFactor * Math.max(strike - forward, 0) };
    }
    const rootT = Math.sqrt(tenor);
    const d1 = (Math.log(forward / strike) + 0.5 * volatility * volatility * tenor) / (volatility * rootT);
  ...
  function metrics(config) {
    ...
    const prices = optionFromForward(forward, config.strike, config.tenor, config.rate, config.volatility);
    const spotForwardPrices = optionFromForward(config.spot, config.strike, config.tenor, config.rate, config.volatility);
    return {
      spot: config.spot,
      rateDiscount,
      dividendDiscount,
  ...
  function forwardCurve(config, maximumTenor, count) {
    const points = [];
  ...
  function optionStrip(config, minimumStrike, maximumStrike, count) {
    const points = [];
    ...
    for (let index = 0; index < total; index += 1) {
      ...
      points.push({ strike, carried: optionFromForward(result.forward, strike, config.tenor, config.rate, config.volatility), spotForward: optionFromForward(config.spot, strike, config.tenor, config.rate, config.volatility) });
    }
    return points;
  }

  const api = { presets, normCdf, optionFromForward, metrics, forwardCurve, optionStrip };
  root.ForwardEngine = api;
