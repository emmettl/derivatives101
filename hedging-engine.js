(function (root) {
  ...
  function callValueDelta(spot, strike, remaining, rate, dividend, volatility) {
    if (remaining <= 0) return { value: Math.max(spot - strike, 0), delta: spot > strike ? 1 : spot < strike ? 0 : 0.5 };
    if (volatility <= 0) {
      const forwardIntrinsic = spot * Math.exp(-dividend * remaining) - strike * Math.exp(-rate * remaining);
      return { value: Math.max(forwardIntrinsic, 0), delta: forwardIntrinsic > 0 ? Math.exp(-dividend * remaining) : 0 };
    }
    const rootT = Math.sqrt(remaining);
    const d1 = (Math.log(spot / strike) + (rate - dividend + 0.5 * volatility * volatility) * remaining) / (volatility * rootT);
    const d2 = d1 - volatility * rootT;
    return {
      value: spot * Math.exp(-dividend * remaining) * normCdf(d1) - strike * Math.exp(-rate * remaining) * normCdf(d2),
  ...
  function normalize(values) {
    ...
    const deviation = Math.sqrt(centered.reduce((sum, value) => sum + value * value, 0) / centered.length) || 1;
    return centered.map((value) => value / deviation);
  }

  function pathShape(kind, steps, seed) {
    const noise = normals(steps, seed);
    if (kind === "selloff") {
      return normalize(noise.map((value, index) => value * 0.8 + (index < 32 ? -1.3 : index < 105 ? 0.45 : 0)));
    }
    if (kind === "late") {
      return normalize(noise.map((value, index) => value * (index < 150 ? 0.3 : 1.65) + (index > 188 && index < 214 ? -0.7 : 0)));
    }
  ...
  function generatePath(config) {
    ...
    const points = [{ day: 0, spot: config.startSpot, logReturn: 0 }];
    ...
    const variance = points.slice(1).reduce((sum, point) => sum + Math.pow(point.logReturn - mean, 2), 0) / steps;
    return { points, realizedVolatility: Math.sqrt(variance * 252) };
  }

  function runHedge(config, costBps) {
    const generated = generatePath(config);
    const points = generated.points;
    const steps = points.length - 1;
    const dt = config.tenor / steps;
    const initialOption = callValueDelta(config.startSpot, config.strike, config.tenor, config.rate, config.dividend, config.impliedVolatility);
    ...
    const records = [{ day: 0, spot: config.startSpot, optionValue: initialOption.value, delta: initialOption.delta, hedge, trade: hedge, transactionCost, portfolio: initialOption.value + hedge * config.startSpot + cash }];

    for (let day = 1; day <= steps; day += 1) {
      cash *= Math.exp(config.rate * dt);
      const remaining = Math.max(0, config.tenor - day * dt);
      const option = callValueDelta(points[day].spot, config.strike, remaining, config.rate, config.dividend, config.impliedVolatility);
      let trade = 0;
      transactionCost = 0;
      ...
      records.push({ day, spot: points[day].spot, optionValue: option.value, delta: option.delta, hedge, trade, transactionCost, portfolio: option.value + hedge * points[day].spot + cash });
    }
    ...
    return { records, initialPremium: initialOption.value, payoff, finalPnl: payoff + cash, totalCosts, turnover, trades, realizedVolatility: generated.realizedVolatility };
  }

  function simulate(config) {
    const withCosts = runHedge(config, config.costBps);
    const withoutCosts = config.costBps ? runHedge(config, 0) : withCosts;
    return Object.assign({}, withCosts, { frictionlessPnl: withoutCosts.finalPnl, costDrag: withCosts.finalPnl - withoutCosts.finalPnl });
  }

  function compare(config) {
    return ["random", "selloff", "late"].map((pathId) => {
      const result = simulate(Object.assign({}, config, { pathId }));
      return { pathId, result };
    });
  }

  const api = { normCdf, callValueDelta, generatePath, simulate, compare };
  root.HedgingEngine = api;
