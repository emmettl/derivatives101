...
export type BasketBasis = "worst" | "average";
export type BasketScenario = "random" | "rally" | "break" | "diverge" | "selloff";
...
export interface BasketPathOptions {
  seed?: number;
  tenor?: number;
  vol?: number;
  volModel?: PathVolatilityModel;
...
const NAMES = ["Asset A", "Asset B", "Asset C"] as const;
function anchor(anchors: readonly Anchor[], t: number): number {
...
const scenarios: Record<Exclude<BasketScenario, "random">, readonly (readonly Anchor[])[]> = {
  rally: [
    [
      [0, 100],
      [0.45, 112],
...
function reference(levels: readonly number[], basis: BasketBasis): number {
  return basis === "average"
    ? levels.reduce((a, b) => a + b, 0) / levels.length
    : Math.min(...levels);
}
function evaluate(paths: Float64Array[], p: BasketParams) {
...
function simulate(p: BasketParams, seed = 1, count = 2000) {
  const returns = [];
  let called = 0,
    loss = 0,
    coupons = 0,
