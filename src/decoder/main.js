...
function renderClauseList() {
  byId("decoder-clause-list").innerHTML = clauses
    .map((clause, index) => {
...
function renderDetail() {
  ...
  byId("resolution-options").innerHTML = clause.options
    .map(
      (option, index) =>
        `<button type="button" class="resolution-option${index === selectedChoice ? " active" : ""}" data-choice="${index}" aria-pressed="${index === selectedChoice}"><span>${escapeHtml(option.tag)}</span><strong>${escapeHtml(option.label)}</strong><small>${escapeHtml(option.rule)}</small></button>`,
    )
    .join("");
  byId("resolution-options")
  ...
  byId("impact-compare").innerHTML =
    `<div><span>Selected interpretation</span><strong>${escapeHtml(chosen.label)}</strong><p>${escapeHtml(chosen.impact)}</p></div><div><span>Other interpretation</span><strong>${escapeHtml(other.label)}</strong><p>${escapeHtml(other.impact)}</p></div>`;
}
...
function flowSteps() {
  ...
  const fixing =
    state.choices[2] === 0
      ? "Postpone within the defined window; otherwise estimate and flag"
      : "Use the preceding valid official close and flag";
  const coupon =
    state.choices[3] === 0
      ? "Pay current coupon plus eligible memory when the test passes"
      : "On a call date, pay current coupon only and cancel memory";
  if (couponFirst) {
    return [
      ...
      [
        ...
        state.choices[0] === 0
          ? "Set persistent state when close ≤ 60"
          : "Set persistent state when close < 60",
      ],
...
function renderFlow() {
  ...
  flow.innerHTML = steps
    .map(
      (step, index) =>
        `<li><span>${String(index + 1).padStart(2, "0")}</span><strong>${escapeHtml(step[0])}</strong><small>${escapeHtml(step[1])}</small></li>`,
    )
    .join("");
  byId("flow-note").innerHTML =
    state.choices[1] === 0
...
function selectedRules() {
  return clauses.map((clause, index) => ({
    ...
    definition: clause.options[state.choices[index]].rule,
  }));
}

function renderSpecification() {
  const settlement =
    state.choices[4] === 0 ? "Physical downside delivery" : "Cash-only downside settlement";
  ...
  byId("spec-overview").innerHTML = [
    ...
    .join("");
  byId("decoder-rule-strip").innerHTML =
    `<b>ACTIVE note</b><i>→</i><span>validated fixing</span><i>→</i><span>state update</span><i>→</i><span>cash-flow tests</span><i>→</i><span>redemption test</span><i>→</i><b>ACTIVE / REDEEMED / MATURED</b>`;
  const staticRules = [
    {
      id: "ID-01",
      name: "Product identity",
      definition:
        "Single-underlying, three-year note; nominal 1,000; initial level and strike normalised to 100 unless a test explicitly overrides strike.",
    },
    {
      id: "DAT-01",
      name: "Scheduled events",
      definition:
        "Quarterly observation dates; first three are non-call; dates falling on a non-trading day move to the next scheduled trading day.",
    },
    {
      id: "MKT-01",
      name: "Market input",
      definition:
        "Use the underlying exchange’s official closing level for the applicable observation day and retain source, timestamp and validity status.",
    },
    {
      id: "CPN-01",
      name: "Coupon condition",
      definition:
        "Current coupon is 2% of nominal. If officialClose ≥ 70, apply the selected memory rule; otherwise add 20 to memoryBalance.",
    },
    {
      id: "CAL-01",
      name: "Autocall condition",
      definition:
        "From observation four onward, call automatically when officialClose ≥ 100, subject to the selected same-day order.",
    },
    {
      id: "MAT-01",
      name: "Maturity branch",
      definition:
        "If still ACTIVE at maturity: repay par when barrierBreached is false or finalLevel ≥ strike; otherwise apply the selected downside settlement.",
    },
  ];
  byId("compiled-rules").innerHTML = staticRules
    .concat(selectedRules())
    .map(
      (rule) =>
        `<tr><td>${escapeHtml(rule.id)}</td><td>${escapeHtml(rule.name)}</td><td>${escapeHtml(rule.definition)}</td></tr>`,
    )
    .join("");
}
...
function acceptanceTests() {
  ...
  tests.push({
    ...
    expected:
      state.choices[0] === 0
        ? "barrierBreached becomes true; maturity delivers 10 units worth 800."
        : "barrierBreached remains false; maturity repays 1,000 cash.",
  });
  tests.push({
    id: "AT-ORD-01",
    ...
    expected:
      state.choices[1] === 0
        ? "book a 20 coupon, then redeem; total cash due is 1,020."
        : "redeem before testing the coupon; total cash due is 1,000.",
  });
  tests.push({
    id: "AT-DIS-01",
    title: "Scheduled fixing is unavailable",
    given:
      "The preceding valid close is 98; the scheduled observation has no official close; the next valid close is 102.",
    when: "The observation engine requests the contractual fixing.",
    expected:
      state.choices[2] === 0
        ? "use 102 on the postponed date, retain a disruption flag and satisfy the call test."
        : "use the preceding close of 98, retain a disruption flag and leave the note active.",
  });
  tests.push({
    id: "AT-MEM-01",
    ...
    expected:
      state.choices[1] === 1
        ...
        : state.choices[3] === 0
          ? "book 60 coupon cash and clear memoryBalance before redemption."
          : "book 20 current coupon, cancel the 40 memory balance and then redeem.",
  });
  tests.push({
    id: "AT-SET-01",
    ...
    expected:
      state.choices[4] === 0
        ? "deliver 10 whole units plus 8.08 cash-in-lieu; total value 808.08."
        : "deliver no units and pay 808.08 cash.",
  });
  return tests;
}

function renderTests() {
  byId("acceptance-tests").innerHTML = acceptanceTests()
    .map(
      (test) =>
        `<article class="acceptance-test"><header><span>${escapeHtml(test.id)}</span><strong>${escapeHtml(test.title)}</strong></header><dl><div><dt>Given</dt><dd>${escapeHtml(test.given)}</dd></div><div><dt>When</dt><dd>${escapeHtml(test.when)}</dd></div><div><dt>Then</dt><dd>${escapeHtml(test.expected)}</dd></div></dl></article>`,
    )
    .join("");
}
...
function renderReady() {
  ...
  byId("ready-grid").innerHTML = items
    .map(
      (item) =>
        `<div><span aria-hidden="true">✓</span><p><strong>${escapeHtml(item[0])}</strong><small>${escapeHtml(item[1])}</small></p></div>`,
    )
    .join("");
}
...
function specificationText() {
  ...
  acceptanceTests().forEach((test) =>
    lines.push(
      `- ${test.id} ${test.title}: GIVEN ${test.given} WHEN ${test.when} THEN ${test.expected}`,
    ),
  );
  lines.push(
    "",
    "Outside scope: corporate actions, tax, issuer default, hedging, secondary-market valuation and jurisdiction-specific legal drafting.",
  );
  return lines.join("\n");
}
...
async function copySpecification() {
  ...
  try {
    ...
    status.textContent =
      "Copy was unavailable in this browser. The compiled rules remain visible above.";
  }
}
...
function renderSummary() {
  ...
  byId("decoder-summary").textContent =
    alternatives === 0
