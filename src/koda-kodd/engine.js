...
function isKnockOut(spot, p) {
  return p.kind === "koda" ? spot >= p.knockOut : spot <= p.knockOut;
}
function isAdverse(spot, p) {
...
function evaluate(path, p) {
  ...
  for (let index = 0; index < observations.length; index++) {
    ...
    const protectedObservation = index < guaranteed,
      ...
      quantity = execute * p.baseUnits * (geared ? p.gearing : 1),
  ...
  const terminationDay = knockedOut ? events.at(-1).day : observations.at(-1),
