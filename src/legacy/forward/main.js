...
(function () {
  ...
  function createControls() {
    byId("forward-controls").innerHTML =
      `<div class="control-block"><span class="control-title">Market &amp; term</span>${controls.slice(0, 4).map(controlMarkup).join("")}</div><div class="control-block"><span class="control-title">Option example</span>${controls.slice(4).map(controlMarkup).join("")}</div>`;
    controls.forEach((control) =>
      byId(`forward-${control.id}`).addEventListener("input", (event) => {
  ...
  function controlMarkup(control) {
    return `<label class="range-control" for="forward-${control.id}"><span>${esc(control.label)} <output id="forward-${control.id}-out"></output></span><input id="forward-${control.id}" type="range" min="${control.min}" max="${control.max}" step="${control.step}" value="${state[control.id]}"></label>`;
  }

  function renderPresets() {
    byId("forward-presets").innerHTML = Object.values(presets)
      .map(
        (preset) =>
  ...
  function syncControls() {
    controls.forEach((control) => {
      byId(`forward-${control.id}`).value = state[control.id];
    });
  }
  function renderOutputs() {
    controls.forEach((control) => {
      byId(`forward-${control.id}-out`).textContent = control.format(state[control.id]);
    });
  }
  ...
  function renderBridge(result) {
    const steps = [
      ...
      [
        ...
        "Value removed for dividends before expiry",
        result.dividendAdjustment >= 0 ? "positive" : "negative",
      ],
      [
        "03",
        "Prepaid forward",
        result.prepaidForward,
        "Pay today for delivery at expiry",
        "prepaid",
      ],
      [
        "04",
        "Financing adjustment",
        result.financingAdjustment,
        "Carry the prepaid amount to expiry",
        result.financingAdjustment >= 0 ? "positive" : "negative",
      ],
      [
        "05",
        "Forward delivery price",
        result.forward,
        "Amount exchanged at the future date",
        "forward",
      ],
    ];
    byId("forward-bridge").innerHTML = steps
      .map(
        (step) =>
          `<div class="${step[4]}"><span>${step[0]}</span><strong>${esc(step[1])}</strong><b>${step[2] >= 0 && (step[4] === "positive" || step[4] === "negative") ? "+" : step[2] < 0 ? "−" : ""}${Math.abs(step[2]).toFixed(2)}</b><p>${esc(step[3])}</p></div>`,
      )
      .join("");
  }
  ...
  function renderCurve() {
    ...
    const path = (key) =>
      curve
        .map(
          (point, indexValue) =>
            `${indexValue ? "L" : "M"}${x(point.tenor).toFixed(2)},${y(point[key]).toFixed(2)}`,
        )
        .join(" ");
  ...
  function renderOptions(result) {
    const cards = [
      [
        `${(result.forwardMoneyness * 100).toFixed(1)}%`,
        "Strike / forward",
        result.forwardMoneyness < 0.995
          ? "Call is in the money versus forward"
          : result.forwardMoneyness > 1.005
            ? "Call is out of the money versus forward"
            : "Call is approximately ATM-forward",
      ],
      [
        result.prices.call.toFixed(2),
        "Call value with carry",
        `${result.prices.put.toFixed(2)} corresponding put value`,
      ],
      [
        `${(result.participation * 100).toFixed(1)}%`,
        "Budget-implied participation",
        `${state.optionBudget.toFixed(2)} budget ÷ ${result.prices.call.toFixed(2)} call cost`,
      ],
    ];
    byId("forward-options").innerHTML = cards
      .map(
        (card, indexValue) =>
          `<article class="option-${indexValue}"><span>${esc(card[1])}</span><strong>${esc(card[0])}</strong><p>${esc(card[2])}</p></article>`,
      )
      .join("");
    const participationChange = result.participation - result.spotForwardParticipation;
    byId("forward-option-note").textContent =
      `If forward were incorrectly set equal to spot, the call would be ${result.spotForwardPrices.call.toFixed(2)} and the same budget would imply ${(result.spotForwardParticipation * 100).toFixed(1)}% participation. Carry therefore changes this illustrative participation by ${participationChange >= 0 ? "+" : "−"}${Math.abs(participationChange * 100).toFixed(1)} percentage points.`;
  ...
  function renderOptionChart(result) {
    ...
    const path = (key) =>
      pointsList
        .map(
          (point, indexValue) =>
            `${indexValue ? "L" : "M"}${x(point.strike).toFixed(2)},${y(point[key].call).toFixed(2)}`,
        )
        .join(" ");
  ...
  function renderParity(result) {
    ...
    const items = [
      [
        "Call minus put",
        result.parityLeft,
        `${result.prices.call.toFixed(4)} − ${result.prices.put.toFixed(4)}`,
      ],
      [
        "Dividend-adjusted spot",
        state.spot * result.dividendDiscount,
        `${state.spot.toFixed(2)} × e^(−qT)`,
      ],
      [
        "Discounted strike",
        state.strike * result.rateDiscount,
        `${state.strike.toFixed(2)} × e^(−rT)`,
      ],
      ["Parity residual", difference, "Left side minus right side"],
    ];
    byId("forward-parity").innerHTML =
      `<div class="forward-parity-equation"><strong>C − P</strong><i>=</i><strong>S e<sup>−qT</sup> − K e<sup>−rT</sup></strong></div><div class="forward-parity-values">${items.map((item, indexValue) => `<div class="${indexValue === 3 ? "residual" : ""}"><span>${esc(item[0])}</span><strong>${Math.abs(item[1]) < 0.00005 ? "0.0000" : signed(item[1], 4)}</strong><p>${esc(item[2])}</p></div>`).join("")}</div>`;
  }

  function renderScenarios() {
    byId("forward-scenarios").innerHTML = Object.values(presets)
      .map((preset) => {
  ...
  function renderRules() {
    const rows = [
      [
        "Spot source",
        "Which market, close, timestamp, currency and adjustment state defines spot?",
        "A stale or differently adjusted spot creates a different basis.",
      ],
      [
        "Financing curve",
        "Which curve, collateral basis, compounding, day count and interpolation apply at the option date?",
        "One flat risk-free rate is only a teaching approximation.",
      ],
      [
        "Dividend model",
        "Are dividends represented as dated cash amounts, a continuous yield or an implied dividend curve?",
        "Timing and amounts matter, especially around discrete ex-dates.",
      ],
      [
        "Borrow and repo",
        "Does stock lending value, hard-to-borrow cost or an implied repo input affect the forward?",
        "Single-stock forwards can depart from a cash-and-carry calculation that ignores borrow.",
      ],
      [
        "Tenor dates",
        "From which settlement date to which expiry or delivery date is carry accrued?",
        "Trade date, spot date, option expiry and payment date are not interchangeable.",
      ],
      [
        "Moneyness convention",
        "Does ATM mean spot, forward, delta-neutral or another market convention?",
        "The selected strike can be ATM under one definition and not another.",
      ],
      [
        "Corporate actions",
        "How are ordinary dividends, special dividends, splits, rights and mergers treated?",
        "Forecast inputs and legal option adjustments are separate decisions.",
      ],
      [
        "Currency and quanto",
        "Are spot, dividends, curves, strike and payout in one currency?",
        "Cross-currency discounting or fixed conversion adds another carry layer.",
      ],
      [
        "Consistency tests",
        "Must put–call parity, forward curves and option values reconcile within a stated tolerance?",
        "A residual can expose mixed timestamps, curves or dividend assumptions.",
      ],
    ];
    byId("forward-rules").innerHTML = rows
      .map(
  ...
  function render() {
    ...
    byId("forward-summary").textContent =
      `At ${percent(state.rate)} financing and ${percent(state.dividendYield)} dividend yield, net carry is ${percent(result.netCarry)}. Over ${state.tenor.toFixed(2)} years, spot ${state.spot.toFixed(2)} becomes a ${result.forward.toFixed(2)} forward—not a forecast, but a no-arbitrage delivery level under these assumptions.`;
    const stats = [
      [percent(result.netCarry), "Net annual carry", "Financing rate minus dividend yield"],
      [
        result.forward.toFixed(2),
        "Forward level",
        `For delivery in ${state.tenor.toFixed(2)} years`,
      ],
      [signed(result.basis), "Forward basis", "Forward minus spot"],
      [
        `${(result.forwardMoneyness * 100).toFixed(1)}%`,
        "Strike / forward",
        state.strike === state.spot
          ? "ATM spot does not imply ATM forward"
          : `${state.strike.toFixed(0)} strike versus ${result.forward.toFixed(2)} forward`,
      ],
    ];
    byId("forward-stats").innerHTML = stats
      .map(
        (stat) =>
  ...
  byId("forward-reset").addEventListener("click", () => {
    state = Object.assign({}, presets.financing);
    syncControls();
    render();
  });
  createControls();
