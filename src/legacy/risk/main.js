...
(function () {
  ...
  let state = {
    ...
    lifeFraction: 0.5,
    spot: 100,
    volatility: products.protected.issueVolatility,
    rate: 0.03,
    issuerSpread: 0.015,
    dividend: 0.02,
  };
  ...
  const controls = [
    {
      ...
      max: 1,
      step: 0.05,
      format: (value, note) => `${(value * note.tenor).toFixed(2)} years`,
    },
    {
      id: "spot",
      label: "Underlying level",
      min: 40,
      max: 160,
      step: 1,
  ...
  function signed(value, digits) {
    return `${value >= 0 ? "+" : "−"}${Math.abs(value).toFixed(digits == null ? 3 : digits)}`;
  }
  function note() {
    return products[state.productId];
  }
  function market() {
    return {
      spot: state.spot,
      remaining: state.lifeFraction * note().tenor,
  ...
  function createControls() {
    byId("risk-controls").innerHTML =
      `<div class="control-block">${controls.slice(0, 2).map(controlMarkup).join("")}</div><div class="control-block"><span class="control-title">Market inputs</span>${controls.slice(2).map(controlMarkup).join("")}</div>`;
  ...
  function renderProducts() {
    byId("risk-products").innerHTML = Object.values(products)
      .map(
        (product) =>
          `<button type="button" data-product="${product.id}" class="${product.id === state.productId ? "on" : ""}" aria-pressed="${product.id === state.productId}">${esc(product.name)}</button>`,
      )
      .join("");
    byId("risk-products")
  ...
  function syncControls() {
    controls.forEach((control) => {
      byId(`risk-${control.id}`).value = state[control.id];
    });
  }

  function renderControlOutputs() {
    controls.forEach((control) => {
      byId(`risk-${control.id}-out`).textContent = control.format(state[control.id], note());
    });
  }

  function renderSummary(risk) {
    const protectedProduct = note().id === "protected";
    byId("risk-headline").textContent = protectedProduct
      ? "Long call · positive curvature"
      : "Short put · negative curvature";
    const signText = protectedProduct ? "positive gamma and vega" : "negative gamma and vega";
    byId("risk-summary").textContent =
      `${note().name} model value ${risk.value.toFixed(2)} at spot ${state.spot.toFixed(0)}. Its ${note().description.toLowerCase()} construction produces ${signText} near the selected point.`;
  }
  ...
  function renderStats(risk) {
    const stats = [
      ...
      [
        signed(risk.vegaOnePoint, 3),
        "Vega · +1 vol point",
        "Value change for volatility +1 percentage point",
      ],
      [signed(risk.oneMonthCarry, 3), "One-month carry", "Value change if only one month passes"],
    ];
    byId("risk-stats").innerHTML = stats
      .map(
        (item) =>
          `<div><span>${esc(item[1])}</span><strong class="${item[0].startsWith("−") ? "event-negative" : ""}">${esc(item[0])}</strong><p>${esc(item[2])}</p></div>`,
      )
      .join("");
    if (note().id === "protected") {
      byId("risk-interpretation").textContent =
        "The long call makes value convex: delta tends to rise as spot rises. Higher implied volatility generally increases the option component.";
    } else {
      byId("risk-interpretation").textContent =
        "The investor is short the put: value still rises with spot, but delta tends to fall as spot rises. Higher implied volatility makes the embedded liability more expensive.";
    }
  }
  ...
  function renderValueChart(curve, currentRisk) {
    ...
    const path = (key) =>
      curve
        .map(
          (point, index) =>
            `${index ? "L" : "M"}${x(point.spot).toFixed(2)},${y(point[key]).toFixed(2)}`,
        )
        .join(" ");
  ...
  function renderComponents(marked) {
    const entries = [
      ...
      ...(Math.abs(marked.components.coupons) > 0.0001
        ? [["Remaining coupons", marked.components.coupons]]
        : []),
    ...
    byId("risk-components").innerHTML = entries
      .map((entry) => {
      ...
      .join("");
    byId("component-total").innerHTML =
      `Component total <strong>${marked.value.toFixed(2)}</strong>. At issue the illustrative estimated value was 98.00.`;
    if (note().id === "protected") {
      byId("risk-build-copy").textContent =
        `The debt component creates the floor. A ${(note().participation * 100).toFixed(1)}% long call creates positive gamma and vega.`;
    } else {
      byId("risk-build-copy").textContent =
        `The ${(note().couponRate * 100).toFixed(2)}% continuous-equivalent coupon helps fund a short put. That liability creates negative gamma and vega.`;
    }
  }
  ...
  function renderShocks(base) {
    ...
    byId("risk-shocks").innerHTML = shocks
      .map((shock) => {
  ...
  function renderRules() {
    const rows = [
      [
        "Delta",
        "Central change in model value for a very small change in spot",
        "It is not the maturity participation rate and is not constant",
      ],
      [
        "Gamma",
        "Central change in delta per one spot unit",
        "A large move contains gamma; delta alone will not predict it",
      ],
      [
        "Vega",
        "Model-value change for implied volatility +1 percentage point",
        "It is not the effect of a 1% relative volatility change",
      ],
      [
        "One-month carry",
        "Revalue with one month less remaining and all market inputs unchanged",
        "Real markets and accrued cash flows rarely stay unchanged",
      ],
      [
        "Finite shock",
        "Fully revalue after the stated change",
        "Shock results can include several interacting sensitivities",
      ],
      [
        "Model scope",
        "European option decomposition with one volatility and one issuer spread",
        "Barriers, autocalls, skew, liquidity and default recovery require richer models",
      ],
    ];
    byId("risk-rules").innerHTML = rows
      .map(
  ...
  byId("risk-reset").addEventListener("click", () => {
    ...
    state = {
      ...
      lifeFraction: 0.5,
      spot: 100,
      volatility: products[productId].issueVolatility,
      rate: 0.03,
      issuerSpread: 0.015,
      dividend: 0.02,
    };
