...
(function () {
  ...
  const controls = [
    {
      group: "Contract clock",
      id: "elapsed",
      label: "Time elapsed",
      min: 0,
      max: 4.75,
      step: 0.25,
      format: (value) => `${value.toFixed(2).replace(/\.00$/, "")} years`,
    },
    {
      group: "Contract clock",
      id: "spot",
    ...
    {
      group: "Potential exit",
      id: "exitCost",
      label: "Bid / unwind deduction",
      min: 0,
      max: 4,
      step: 0.1,
      format: (value) => `${value.toFixed(1)} points`,
    },
  ];
  ...
  function createControls() {
    ...
    byId("valuation-controls").innerHTML = groups
      .map(
        (group) =>
          `<div class="control-block"><span class="control-title">${esc(group.name)}</span>${group.items.map((control) => `<label class="range-control" for="valuation-${control.id}"><span>${esc(control.label)} <output id="valuation-${control.id}-out"></output></span><input id="valuation-${control.id}" type="range" min="${control.min}" max="${control.max}" step="${control.step}" value="${market[control.id]}"></label>`).join("")}</div>`,
      )
      .join("");
    controls.forEach((control) => {
  ...
  function renderScenarioButtons() {
    byId("valuation-scenarios").innerHTML = scenarios
      .map(
        (scenario) =>
          `<button type="button" data-scenario="${scenario.id}" class="${scenario.id === activeScenario ? "on" : ""}" aria-pressed="${scenario.id === activeScenario}">${esc(scenario.label)}</button>`,
      )
      .join("");
    byId("valuation-scenarios")
  ...
  function renderSummary(marked, attribution) {
    ...
    byId("valuation-headline").textContent =
      Math.abs(spotMove) < 0.0001
        ? `Spot unchanged at ${market.spot.toFixed(0)}`
        : `Spot ${spotMove > 0 ? "up" : "down"} ${Math.abs(spotMove * 100).toFixed(0)}%`;
    const largest = attribution.steps.reduce(
      (best, step) => (Math.abs(step.change) > Math.abs(best.change) ? step : best),
      attribution.steps[0],
    );
    byId("valuation-summary").textContent =
      `After ${market.elapsed.toFixed(2).replace(/\.00$/, "")} years, the illustrative bid is ${points(marked.bid)} versus an issue price of 100. The largest step in this ordered bridge is ${largest.label.toLowerCase()} (${signed(largest.change)}).`;
  }
  ...
  function renderIssueAndStack(marked) {
    byId("issue-strip").innerHTML = [
      ...
      [
        ...
        `${(note.participation * 100).toFixed(1)}%`,
        "Fixed when the note is issued",
      ],
    ]
      .map(
        (item) =>
    ...
    stack.querySelector(".value-stack-bond").style.width =
      `${Math.max(0, (marked.bond / scaleMax) * 100)}%`;
    stack.querySelector(".value-stack-option").style.width =
      `${Math.max(0, (marked.option / scaleMax) * 100)}%`;
    stack.querySelector(".issue-price-marker").style.left =
      `${(note.issuePrice / scaleMax) * 100}%`;
    stack.setAttribute(
      "aria-label",
      `Current model value ${points(marked.modelValue)}: issuer bond ${points(marked.bond)} plus option ${points(marked.option)}. Illustrative bid after deduction ${points(marked.bid)}.`,
    );
    byId("value-stack-labels").innerHTML = [
      ...
      .map((item) => `<span><b>${item[0]}</b>${item[1]}</span>`)
      .join("");
    const difference = marked.bid - note.issuePrice;
    byId("valuation-message").innerHTML =
      difference < 0
  ...
  function renderCurve(marked) {
    ...
    const width = 900,
      ...
      bottom = 48;
    const values = data
      .flatMap((point) => [point.modelValue, point.bid, point.maturityPayoff])
      .concat([100]);
    ...
    byId("valuation-comparison").innerHTML = [
      [
        ...
        "Maturity payoff at today’s spot",
        "Only if held to maturity and the issuer performs",
      ],
      [
        points(marked.modelValue),
        "Current model value",
        `${marked.remaining.toFixed(2)} years remain`,
      ],
      [
        points(marked.bid),
        "Illustrative exit bid",
        `After a ${market.exitCost.toFixed(1)} point deduction`,
      ],
    ]
      .map(
        (item) =>
  ...
  function renderDrivers(attribution) {
    ...
    byId("valuation-drivers").innerHTML = attribution.steps
      .map((step) => {
      ...
      .join("");
    byId("attribution-note").textContent =
      `Ordered bridge: ${points(attribution.start)} initial estimated value → ${points(attribution.end)} illustrative bid. Contributions depend on the order because valuation inputs interact.`;
  }
  ...
  function renderRules() {
    const rows = [
      [
        "Maturity payoff",
        `100 + ${(note.participation * 100).toFixed(1)}% × max(final level − 100, 0)`,
        "Issuer default, recovery and tax",
      ],
      [
        "Issue-date value",
        "Issue price 100 − upfront difference 2 = estimated value 98",
        "Actual commissions, hedging profit and issuer methodology",
      ],
      [
        "Issuer bond",
        "100 × exp[−(reference rate + issuer spread) × remaining tenor]",
        "Full credit curve, recovery and funding-basis effects",
      ],
      [
        "Embedded option",
        "Fixed participation × Black–Scholes call × issuer-spread discount factor",
        "Volatility skew, discrete dividends, jumps and model calibration",
      ],
      [
        "Current model value",
        "Issuer bond + embedded option",
        "Dealer reserves, size, liquidity and competing models",
      ],
      [
        "Indicative bid",
        "max(current model value − selected exit deduction, 0)",
        "No commitment to transact; an actual quote may differ or be unavailable",
      ],
    ];
    byId("valuation-rules").innerHTML = rows
      .map(
