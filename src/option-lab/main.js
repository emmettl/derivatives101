...
function updateReadouts() {
  const p = params(),
    m = optionMetrics(p.S, p.K, p.T, p.r, p.q, p.v);
  $("spot-out").textContent = fmt(p.S, 0);
  $("strike-out").textContent = fmt(p.K, 0);
  $("vol-out").textContent = fmt(p.v * 100, 0) + "%";
  ...
  $("delta").textContent = fmt(m.delta, 3);
  $("gamma").textContent = fmt(m.gamma, 4);
  $("vega").textContent = fmt(m.vega / 100, 3);
  $("theta").textContent = fmt(m.theta / 365, 3);
  return { p, m };
}
...
function drawSurface() {
  ...
  if (surfaceWorker) {
    ...
    canvas.setAttribute(
      "aria-label",
      `${state.type} option value surface with spot ${p.S}, strike ${p.K}, volatility ${fmt(p.v * 100, 0)} percent and ${fmt(p.T, 2)} years to expiry`,
    );
    return;
  }
  ...
  for (let j = 0; j <= nt; j++) {
    ...
    for (let i = 0; i <= nx; i++) {
      const S = sLo + ((sHi - sLo) * i) / nx,
        T = (p.T * j) / nt,
        m = optionMetrics(S, p.K, T, p.r, p.q, p.v);
      row.push({
        S,
        T,
        v: m.price,
  ...
  cells.forEach(({ i, j }) => {
    ...
    const t = Math.min(1, tv / (p.K * 0.14));
    ctx.beginPath();
    q.forEach((x, k) => (k ? ctx.lineTo(...x.pt) : ctx.moveTo(...x.pt)));
    ctx.closePath();
    ctx.fillStyle = surfaceColors[Math.round(t * 63)];
    ctx.globalAlpha = 0.82;
    ctx.fill();
    ctx.globalAlpha = 0.42;
    ctx.strokeStyle = "#6f919e";
    ctx.lineWidth = 0.45;
    ctx.stroke();
    ctx.globalAlpha = 1;
  });
  [0, nt].forEach((j) => {
    ctx.beginPath();
    points[j].forEach((x, i) => (i ? ctx.lineTo(...x.pt) : ctx.moveTo(...x.pt)));
    ctx.strokeStyle = j ? palette.amber : palette.white;
    ctx.lineWidth = j ? 2 : 2.5;
    ctx.stroke();
  });
  ...
  const current = optionMetrics(p.S, p.K, p.T, p.r, p.q, p.v),
    ix = (p.S - sLo) / (sHi - sLo),
  ...
  ctx.beginPath();
  ctx.arc(marker[0], marker[1], 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "#071620";
  ctx.lineWidth = 2;
  ctx.stroke();
  ...
  ctx.fillText("lower spot", expiryLeft[0] - 14, expiryLeft[1] + 24);
  ctx.fillText("higher spot", expiryRight[0] - 35, expiryRight[1] + 24);
  ctx.fillStyle = palette.white;
  ctx.fillText("EXPIRY PAYOFF", expiryLeft[0], expiryLeft[1] - 13);
  ctx.fillStyle = palette.amber;
  ctx.fillText("TODAY", todayLeft[0], todayLeft[1] - 13);
  canvas.setAttribute(
    "aria-label",
    `${state.type} option value surface with spot ${p.S}, strike ${p.K}, volatility ${fmt(p.v * 100, 0)} percent and ${fmt(p.T, 2)} years to expiry`,
  );
...
const greekInfo = {
  delta: [
    "Delta",
    "Delta is the slope of the value curve: near zero for an out-of-the-money call, near one when it is deep in the money.",
  ],
  gamma: [
    "Gamma",
    "Gamma concentrates around the strike and sharpens as expiry approaches. This is where delta changes fastest.",
  ],
  vega: [
    "Vega",
    "Vega is largest near the strike when there is still time left. Volatility matters most when the outcome remains genuinely uncertain.",
  ],
  theta: [
    "Theta",
    "Theta is usually most negative near the strike. Time decay is not a steady fee; it accelerates where time value is concentrated.",
  ],
  rho: [
    "Rho",
    "Rho shows sensitivity to rates. It grows with time and has the opposite sign for puts and calls.",
  ],
};
function greekValue(m, g) {
  return metricValue(m, g);
}
let greekPlot = null;
function drawGreeks() {
  const canvas = $("greeks"),
  ...
  for (let i = 0; i <= n; i++) {
    const S = lo + ((hi - lo) * i) / n,
      m = optionMetrics(S, p.K, p.T, p.r, p.q, p.v);
    vals.push({ S, y: greekValue(m, state.greek) });
  }
  let ymin = Math.min(0, ...vals.map((x) => x.y)),
    ymax = Math.max(0, ...vals.map((x) => x.y));
  if (ymax - ymin < 1e-8) {
    ymin = -1;
    ymax = 1;
  }
  ...
  ymin -= pad;
  ymax += pad;
  const m = { l: 58, r: 24, t: 25, b: 42 },
    X = (s) => m.l + ((s - lo) / (hi - lo)) * (w - m.l - m.r),
    Y = (y) => m.t + ((ymax - y) / (ymax - ymin)) * (h - m.t - m.b);
  ctx.clearRect(0, 0, w, h);
  ...
  for (let i = 0; i <= 6; i++) {
    const x = lo + ((hi - lo) * i) / 6;
    ctx.strokeStyle = "rgba(190,211,220,.10)";
    ctx.beginPath();
    ctx.moveTo(X(x), m.t);
    ctx.lineTo(X(x), h - m.b);
    ctx.stroke();
    ctx.fillStyle = palette.muted;
    ctx.fillText(fmt(x, 0), X(x), h - 17);
  }
  for (let i = 0; i <= 4; i++) {
    ...
    ctx.textAlign = "right";
    ctx.fillStyle = palette.muted;
    ctx.fillText(Math.abs(y) < 0.01 ? fmt(y, 3) : fmt(y, 2), m.l - 8, Y(y) + 3);
    ctx.textAlign = "center";
  }
  ctx.strokeStyle = "rgba(237,243,246,.45)";
  ctx.setLineDash([4, 4]);
  ctx.beginPath();
  ctx.moveTo(m.l, Y(0));
  ...
  grad.addColorStop(0, "rgba(228,163,64,.38)");
  grad.addColorStop(1, "rgba(62,142,126,.03)");
  ctx.beginPath();
  vals.forEach((x, i) => (i ? ctx.lineTo(X(x.S), Y(x.y)) : ctx.moveTo(X(x.S), Y(x.y))));
  ctx.lineTo(X(hi), Y(0));
  ctx.lineTo(X(lo), Y(0));
  ctx.closePath();
  ctx.fillStyle = grad;
  ...
  [
    ...
    ctx.beginPath();
    ctx.moveTo(X(s), m.t);
    ctx.lineTo(X(s), h - m.b);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = col;
    ctx.fillText(label, X(s), 14);
  });
  greekPlot = { X, Y, lo, hi, ymin, ymax, vals, m, w, h };
  canvas.setAttribute(
    "aria-label",
    `${greekInfo[state.greek][0]} profile across spot from ${fmt(lo, 0)} to ${fmt(hi, 0)}`,
  );
  $("greek-insight-label").textContent = greekInfo[state.greek][0];
...
function drawPaths(sim) {
  const canvas = $("paths"),
    { ctx, w, h } = resize(canvas),
    p = sim.p,
    m = { l: 48, r: 25, t: 24, b: 38 },
    split = w * 0.73,
    pathW = split - m.l - 18;
  const all = sim.samples.flatMap((path) => Array.from(path)),
    rawLo = Math.min(p.K, p.S, ...all),
    rawHi = Math.max(p.K, p.S, ...all),
    pad = (rawHi - rawLo) * 0.08,
    ylo = Math.max(0, rawLo - pad),
    yhi = rawHi + pad;
  const X = (i) => m.l + (i / sim.steps) * pathW,
    Y = (s) => m.t + ((yhi - s) / (yhi - ylo)) * (h - m.t - m.b);
  ctx.clearRect(0, 0, w, h);
  ctx.fillStyle = palette.night;
  ctx.fillRect(0, 0, w, h);
  ctx.font = "10px Segoe UI, sans-serif";
  for (let i = 0; i <= 4; i++) {
    const s = ylo + ((yhi - ylo) * i) / 4;
    ctx.strokeStyle = "rgba(190,211,220,.1)";
    ctx.beginPath();
    ctx.moveTo(m.l, Y(s));
    ctx.lineTo(split - 16, Y(s));
    ctx.stroke();
    ctx.fillStyle = palette.muted;
    ctx.textAlign = "right";
    ctx.fillText(fmt(s, 0), m.l - 7, Y(s) + 3);
  }
  sim.samples.forEach((path, index) => {
    const itm = sim.type === "call" ? path.at(-1) > p.K : path.at(-1) < p.K,
      focus = index < 10;
    ctx.beginPath();
    path.forEach((s, i) => (i ? ctx.lineTo(X(i), Y(s)) : ctx.moveTo(X(i), Y(s))));
    ctx.strokeStyle = itm
      ? focus
        ? "rgba(91,190,169,.78)"
        : "rgba(62,142,126,.46)"
      : focus
        ? "rgba(159,191,209,.68)"
        : "rgba(117,148,166,.34)";
    ctx.lineWidth = focus ? 1.35 : 1;
    ctx.stroke();
  });
  ctx.strokeStyle = palette.amber;
  ctx.setLineDash([5, 4]);
  ctx.beginPath();
  ctx.moveTo(m.l, Y(p.K));
  ctx.lineTo(split - 16, Y(p.K));
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillStyle = palette.amber;
  ctx.textAlign = "left";
  ctx.fillText("strike", m.l + 5, Y(p.K) - 6);
  ctx.fillStyle = palette.muted;
  ctx.textAlign = "center";
  ctx.fillText("today", m.l, h - 15);
  ctx.fillText("expiry", split - 22, h - 15);
  const bins = 26,
...
const mcWorker =
  typeof Worker !== "undefined"
    ? new Worker(new URL("./option-worker.ts", import.meta.url), { type: "module" })
    : null;
function setMCStatus(message, busy = false) {
...
function renderFast() {
  ...
  drawSurface();
  drawGreeks();
  lastPlotAt = performance.now();
}
function renderPlots() {
  if (!surfaceWorker) drawSurface();
  drawGreeks();
  lastPlotAt = performance.now();
}
...
function scheduleFastRender() {
  cancelAnimationFrame(fastFrame);
  fastFrame = requestAnimationFrame(() => {
    updateReadouts();
    if (surfaceWorker) drawSurface();
  });
  queuePlotRender();
  clearTimeout(settledPlotTimer);
  settledPlotTimer = setTimeout(() => queuePlotRender(true), 100);
}
function finishSimulation(sim) {
  if (sim.id !== mcVersion) return;
  lastSimulation = sim;
  drawPaths(sim);
  $("paths").classList.remove("refreshing");
  setMCStatus(`Sample ${sim.sample} · current`);
}
...
function scheduleSimulation(delay = 180) {
  ...
  $("paths").classList.add("refreshing");
  setMCStatus(`Sample ${state.sample} · ${delay ? "waiting…" : "calculating…"}`, true);
...
document.querySelectorAll("#option-type button").forEach((button) =>
  button.addEventListener("click", () => {
    state.type = button.dataset.value;
    document.querySelectorAll("#option-type button").forEach((x) => {
      const on = x === button;
...
document.querySelectorAll("#greek-picker button").forEach((button) =>
  button.addEventListener("click", () => {
    state.greek = button.dataset.greek;
    document
      .querySelectorAll("#greek-picker button")
...
if (typeof Worker !== "undefined" && typeof surface.transferControlToOffscreen === "function") {
  try {
    ...
    surfaceWorker.onmessage = (event) => {
      if (event.data.action === "rendered") surface.dataset.frame = event.data.frame;
    };
    surface.dataset.renderer = "worker";
  } catch {
    surfaceWorker = null;
  }
}
...
surface.addEventListener("pointermove", (e) => {
  ...
  state.yaw = state.drag.yaw + (e.clientX - state.drag.x) / 220;
  state.pitch = Math.max(0.18, Math.min(1, state.drag.pitch + (e.clientY - state.drag.y) / 300));
  cancelAnimationFrame(surfaceFrame);
  surfaceFrame = requestAnimationFrame(drawSurface);
});
surface.addEventListener("pointerup", () => (state.drag = null));
surface.addEventListener("pointercancel", () => (state.drag = null));
...
$("greeks").addEventListener("pointermove", (e) => {
  ...
  const rect = e.currentTarget.getBoundingClientRect(),
    ...
    p = greekPlot;
  if (x < p.m.l || x > p.w - p.m.r) {
    $("greek-readout").style.display = "none";
    return;
  }
  const S = p.lo + ((x - p.m.l) / (p.w - p.m.l - p.m.r)) * (p.hi - p.lo),
    pr = params(),
    value = greekValue(optionMetrics(S, pr.K, pr.T, pr.r, pr.q, pr.v), state.greek),
    read = $("greek-readout");
  read.textContent = `Spot ${fmt(S, 1)} · ${greekInfo[state.greek][0]} ${fmt(value, 4)}`;
  read.style.display = "block";
  read.style.left = Math.min(x + 10, p.w - 180) + "px";
  read.style.top = "18px";
});
...
$("reset").addEventListener("click", () => {
  state.type = "call";
  document.querySelector('#option-type button[data-value="call"]').click();
  apply(defaults);
...
document.querySelectorAll("[data-preset]").forEach((button) =>
  button.addEventListener("click", () => {
    ...
    if (p === "expiry")
      apply(
        { spot: 100, strike: 100, expiry: 0.08, vol: 25 },
        "Gamma sharpens and theta accelerates around the strike as the remaining time collapses.",
      );
    if (p === "vol")
      apply(
        { spot: 100, strike: 100, expiry: 1, vol: 50 },
        "The surface lifts even though the expected direction has not changed: optionality benefits from dispersion.",
      );
    if (p === "deep")
      apply(
        state.type === "call"
          ? { spot: 140, strike: 90, expiry: 1, vol: 25 }
          : { spot: 60, strike: 110, expiry: 1, vol: 25 },
        "Deep in the money, value behaves more like the underlying and gamma and time value fade.",
      );
  }),
);
...
function setupUrl() {
  ...
  u.searchParams.set("s", p.S);
  u.searchParams.set("k", p.K);
  u.searchParams.set("v", p.v * 100);
  u.searchParams.set("t", p.T);
  u.searchParams.set("r", p.r * 100);
  u.searchParams.set("q", p.q * 100);
  u.searchParams.set("g", state.greek);
  return u.href;
}
$("share-link").addEventListener("click", async () => {
  ...
  setTimeout(() => (button.textContent = "Copy setup link"), 1600);
});
function restore() {
  const q = new URLSearchParams(location.search),
    map = { s: "spot", k: "strike", v: "vol", t: "expiry", r: "rate", q: "dividend" };
  Object.entries(map).forEach(([key, id]) => {
    if (q.has(key)) {
      const n = Number(q.get(key)),
        el = $(id);
      if (Number.isFinite(n) && n >= +el.min && n <= +el.max) el.value = n;
    }
  });
  if (["call", "put"].includes(q.get("type")))
    document.querySelector(`#option-type button[data-value="${q.get("type")}"]`).click();
  if (greekInfo[q.get("g")])
    document.querySelector(`#greek-picker button[data-greek="${q.get("g")}"]`).click();
}
