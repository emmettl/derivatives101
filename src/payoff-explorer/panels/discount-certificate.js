...
(function () {
  ...
  function run() {
    const cap = +$("dc-c").value,
      ...
      months = +$("dc-t").value,
      vol = +$("dc-v").value / 100;
    $("dc-c-v").textContent = cap + "%";
    $("dc-p-v").textContent = px + "%";
    $("dc-b-v").textContent = bar + "%";
    $("dc-t-v").textContent = months;
    $("dc-v-v").textContent = (vol * 100).toFixed(0) + "%";
    const isBar = mode === "barrier";

    /* redemption function, given whether the barrier survived */
    const redeem = (S, survived) =>
      isBar ? (survived ? cap : Math.min(S, cap)) : Math.min(S, cap);

    const lo = 20,
      hi = Math.max(140, cap + 30);
    const f = frame($("dc-pay"), {
      H: 320,
      xr: [lo, hi],
      yr: [0, hi],
    ...
    f.vline(cap, C.steel);
    if (isBar) {
      f.rect(lo, bar, C.brick, 0.05);
      f.vline(bar, C.amberD);
      f.line(
        [
          [bar, cap],
          [hi, cap],
      ...
      f.text(
        Math.min(hi - 14, cap + 22),
        cap + hi * 0.07,
        "barrier intact: full cap",
        C.jade,
        10.5,
      );
      f.text(lo + 26, lo + hi * 0.1, "barrier broken", C.deep, 10.5);
    } else {
      f.line(
        [
          [lo, lo],
    ...
    $("dc-terms").innerHTML =
      "<tr><td>Discount to spot</td><td>" +
      fmt(100 - px, 1) +
      " points</td></tr>" +
      "<tr><td>Maximum return</td><td>" +
      fmt(maxRet, 1) +
      "%</td></tr>" +
      "<tr><td>Annualised maximum</td><td>" +
      fmt((maxRet * 12) / months, 1) +
      "%</td></tr>" +
      "<tr><td>Distance to cap</td><td>" +
      fmt(cap - 100, 1) +
      " points</td></tr>";
    $("dc-read").innerHTML =
      "The most this can ever pay is <b>" +
      fmt(maxRet, 1) +
      "%</b>, reached whenever the underlying finishes at or above " +
      ...
      (cap < 100
        ...
        : "") +
      "In these markets it beat a direct holding <b>" +
      pct(beat / paths, 0) +
      "</b> of the time, and the worst 5% still returned <b>" +
      fmt(q(0.05), 0) +
      "%</b>. The cushion is only " +
      fmt(100 - px, 0) +
      " points — small, but certain" +
      (isBar
        ? ", on top of a conditional barrier that is worth much more when it survives and nothing when it does not."
        : ".");
  }
  run();
})();
