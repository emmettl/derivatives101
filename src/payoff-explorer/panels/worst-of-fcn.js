...
(function () {
  ...
  function simulate(nNames, rho, vol, months, strike, bar, ac, cpn, seedv) {
    ...
    for (let p = 0; p < paths; p++) {
      const S = Array.from({ length: nNames }, () => 100),
        mn = Array.from({ length: nNames }, () => 100);
      let z = [],
        zi = 2,
        done = false,
        ret = 0,
      ...
      for (let d = 1; d <= steps; d++) {
        ...
        if (d % 21 === 0 && d >= 21 && d < steps) {
          let all = true;
          for (let j = 0; j < nNames; j++) if (S[j] < ac) all = false;
          if (all) {
            ret = cpn * (d / 252) * 100;
            life = d / 21;
            called++;
      ...
      if (!done) {
        let worst = 1e9,
          worstMin = 1e9;
        for (let j = 0; j < nNames; j++) {
          if (S[j] < worst) worst = S[j];
          if (mn[j] < worstMin) worstMin = mn[j];
        }
        const breached = worstMin < bar;
        const c = cpn * (months / 12) * 100;
        if (breached) ki++;
        if (breached && worst < strike) {
          ret = (worst / strike) * 100 - 100 + c;
          deliv++;
        } else ret = c;
        life = months;
      }
  ...
  function run() {
    const n = +$("fc-n").value,
      ...
      months = +$("fc-t").value,
      vol = +$("fc-v").value / 100;
    $("fc-n-v").textContent = n;
    $("fc-r-v").textContent = fmt(rho, 2);
    $("fc-c-v").textContent = fmt(cpn * 100, 1) + "%";
    $("fc-k-v").textContent = k + "%";
    $("fc-b-v").textContent = b + "%";
    $("fc-a-v").textContent = ac + "%";
    $("fc-t-v").textContent = months;
    $("fc-v-v").textContent = (vol * 100).toFixed(0) + "%";
    ...
    const lo = Math.max(-100, q(0.002) - 3),
      hi = Math.min(60, q(0.999) + 3);
    histogram($("fc-hist"), R.rets, {
      lo: lo,
      hi: hi,
      split: 0,
      bins: 38,
      xfmt: (v) => v.toFixed(0),
      xlab: "Total return per 100 invested over " + months + " months",
    ...
    const f = frame($("fc-corr"), {
      H: 260,
      xr: [0.4, 5.6],
      yr: [0, maxL],
      xticks: [1, 2, 3, 4, 5],
      xfmt: (v) => v + (v === 1 ? " name" : " names"),
      yticks: ticks(0, maxL, 4),
      yfmt: (v) => v.toFixed(0) + "%",
    ...
    $("fc-read").innerHTML =
      "At " +
      fmt(cpn * 100, 1) +
      "% a year the coupon is <b>unconditional</b> — it is paid in every one of these markets. " +
      "The risk is entirely in the redemption: the note settled into shares <b>" +
      pct(R.deliv / R.paths, 0) +
      "</b> of the time, and the worst 5% of markets returned <b>" +
      fmt(q(0.05), 0) +
      " per 100</b>. " +
      "Adding names raises the loss probability from <b>" +
      fmt(sweep[0][1], 0) +
      "%</b> with one name to <b>" +
      fmt(sweep[4][1], 0) +
      "%</b> with five, at an unchanged coupon — which is exactly why the market typically pays more for a " +
