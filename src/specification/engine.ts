export type ReferenceMeasure = "worst" | "average" | "unresolved";
export type CallComparison = "gte" | "gt" | "unresolved";
export type YesNoChoice = "yes" | "no" | "unresolved";
export type EventOrder = "coupon-first" | "call-first" | "unresolved";
export type FinalConvention = "maturity-only" | "final-call" | "unresolved";
export type Settlement = "cash" | "physical" | "unresolved";
...
export interface SpecificationConfig {
  ...
  finalConvention: FinalConvention;
...
type ChoiceKey = keyof Pick<
  SpecificationConfig,
  "referenceMeasure" | "callComparison" | "memory" | "eventOrder" | "finalConvention" | "settlement"
>;
...
const profiles: Record<string, SpecificationConfig> = {
  resolved: {
    id: "resolved",
...
const scenarios: Record<string, SpecificationScenario> = {
  lateCall: {
    id: "lateCall",
...
const choiceDefinitions: ChoiceDefinition[] = [
  {
...
function referenceLevel(a: number, b: number, measure: ReferenceMeasure): number {
  return measure === "average" ? (a + b) / 2 : Math.min(a, b);
}

function comparisonPass(value: number, threshold: number, operator: CallComparison): boolean {
  return operator === "gt" ? value > threshold : value >= threshold;
}

function compile(config: SpecificationConfig): CompilationReadiness {
...
function evaluate(config: SpecificationConfig, scenario: SpecificationScenario) {
...
function normalizedRows(config: SpecificationConfig): string[][] {
...
function acceptanceTests(config: SpecificationConfig) {
