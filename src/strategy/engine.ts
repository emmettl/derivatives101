import type { BarrierType, LegOutcome, Market, OptionLeg, OptionType, PricingResult, RiskMeasures, ScenarioRow, StrategyMetrics, StrategyOutcome } from "./types";
...
export function vanillaPrice(market: Market, optionType: OptionType, strike: number, spotOverride?: number): number {
...
export function barrierHit(optionLeg: OptionLeg, terminalSpot: number, observedLow: number, observedHigh: number, initialSpot: number): boolean {
...
export function legOutcome(market: Market, optionLeg: OptionLeg, terminalSpot: number, observedLow: number, observedHigh: number): LegOutcome {
  if (!optionLeg.enabled) return { payoff: 0, pnl: 0, signedQuantity: 0, premium: 0, hit: false, active: false, pricing: null };
...
export function strategyOutcome(market: Market, legs: OptionLeg[], terminalSpot: number, observedLow: number, observedHigh: number): StrategyOutcome {
...
export function strategyCurve(market: Market, legs: OptionLeg[], observedLow: number, observedHigh: number, minimum: number, maximum: number, count = 241): StrategyOutcome[] {
...
export function riskMeasures(market: Market, legs: OptionLeg[]): RiskMeasures {
  ...
  const volatilityUp = strategyValue({ ...market, volatility: market.volatility + volatilityBump }, legs);
  const volatilityDown = strategyValue({ ...market, volatility: Math.max(0.0001, market.volatility - volatilityBump) }, legs);
  const valueInThirtyDays = strategyValue({ ...market, tenor: Math.max(0.000001, market.tenor - 30 / 365) }, legs);
  return {
    value,
...
export function scenarioMatrix(market: Market, legs: OptionLeg[], spotMoves: number[], volatilityMoves: number[]): ScenarioRow[] {
  ...
  return spotMoves.map(spotMove => ({
    ...
    cells: volatilityMoves.map(volatilityMove => {
      ...
      return { volatilityMove, volatility: shockedMarket.volatility, value, pnl: value - entryValue };
    })
  }));
}
...
export function metrics(market: Market, legs: OptionLeg[], observedLow: number, observedHigh: number, selectedTerminal: number): StrategyMetrics {
