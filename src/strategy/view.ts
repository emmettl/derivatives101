...
export function createMarketControls(
  state: StrategyState,
  onInput: (key: MarketControl["key"], value: number) => void,
): void {
  byId("market-controls").innerHTML =
    `<div class="control-block"><span class="control-title">Pricing inputs</span>${marketControls
      .slice(0, 5)
      .map((control) => controlMarkup(control, state))
      .join(
        "",
      )}</div><div class="control-block"><span class="control-title">Path &amp; outcome</span>${marketControls
      .slice(5)
      .map((control) => controlMarkup(control, state))
      .join(
        "",
      )}<p class="control-help">The observed low and high record barrier touches before expiry. The selected terminal level is added to that path history.</p></div>`;
  marketControls.forEach((control) =>
    byId<HTMLInputElement>(`strategy-${control.key}`).addEventListener("input", (event) =>
      onInput(control.key, Number((event.currentTarget as HTMLInputElement).value)),
    ),
  );
}
...
function syncMarketControls(state: StrategyState): void {
  ...
  marketControls.forEach((control) => {
    ...
    byId<HTMLOutputElement>(`strategy-${control.key}-out`).textContent = control.format(
      state[control.key],
    );
  });
}
...
function renderSummary(state: StrategyState, metrics: StrategyMetrics): string {
  ...
  const stats = [
    [
      `${debit >= 0 ? "Debit " : "Credit "}${Math.abs(debit).toFixed(2)}`,
      "Net premium",
      "Long premium minus short premium",
    ],
    [
      metrics.breakEvens.length
        ? metrics.breakEvens.map((value) => value.toFixed(1)).join(" / ")
        : "None in range",
      "Break-even level(s)",
      "Across 40%–160% of initial spot",
    ],
    [metrics.minimumPnl.toFixed(2), "Lowest P/L shown", "Within the displayed range"],
    [metrics.maximumPnl.toFixed(2), "Highest P/L shown", "Within the displayed range"],
  ];
  byId("strategy-stats").innerHTML = stats
    .map(
      (item) =>
  ...
  const barrierEvents = metrics.selected.legs
    .map((result, index) =>
      state.legs[index].enabled && state.legs[index].barrierType !== "none"
        ? `Leg ${index + 1} ${result.hit ? "touched" : "did not touch"} its ${state.legs[index].barrierType} barrier and is ${result.active ? "active" : "inactive"}`
        : "",
    )
    .filter(Boolean);
  byId("selected-outcome").textContent =
    `At expiry ${state.terminal.toFixed(0)}, the strategy payoff before premium is ${metrics.selected.payoff.toFixed(2)} and profit/loss after premium is ${metrics.selected.pnl.toFixed(2)}.${barrierEvents.length ? ` ${barrierEvents.join("; ")}.` : ""}`;
  return title;
}

function renderRisk(risk: RiskMeasures, scenarios: ScenarioRow[]): void {
  const stats = [
    [risk.value.toFixed(2), "Model value", "Signed value of every active leg"],
    [signed(risk.delta, 3), "Delta", "Value change for a 1-point spot rise"],
    [signed(risk.gamma, 3), "Gamma", "How delta bends as spot changes"],
    [signed(risk.vega, 2), "Vega", "Approximate change for +1 vol point"],
  ];
  byId("strategy-risk-stats").innerHTML = stats
    .map(
      (item) =>
        `<div><span>${esc(item[1])}</span><strong>${esc(item[0])}</strong><p>${esc(item[2])}</p></div>`,
    )
    .join("");
  const deltaText =
    Math.abs(risk.delta) < 0.05
      ? "roughly direction-neutral here"
      : risk.delta > 0
        ? "benefits initially from a small spot rise"
        : "benefits initially from a small spot fall";
  const gammaText =
    Math.abs(risk.gamma) < 0.005
      ? "has little local curvature"
      : risk.gamma > 0
        ? "is locally convex"
        : "is locally concave";
  const vegaText =
    Math.abs(risk.vega) < 0.05
      ? "has little local volatility sensitivity"
      : risk.vega > 0
        ? "benefits from higher implied volatility"
        : "benefits from lower implied volatility";
  byId("strategy-risk-note").textContent =
    `At the selected inputs the package is ${deltaText}, ${gammaText}, and ${vegaText}. With spot and volatility unchanged, the model value changes by ${signed(risk.theta30, 2)} over the next 30 days.`;
  const volatilityMoves = scenarios[0]?.cells.map((cell) => cell.volatilityMove) ?? [];
  byId("strategy-scenarios").innerHTML =
...
function renderAnatomy(state: StrategyState, metrics: StrategyMetrics): void {
  const activeResults = metrics.selected.legs
    .map((result, index) => ({ result, item: state.legs[index], index }))
    .filter((entry) => entry.item.enabled);
  const maximum = Math.max(1, ...activeResults.map((entry) => Math.abs(entry.result.pnl)));
  byId("contribution-bars").innerHTML =
    activeResults
      .map((entry) => {
  ...
  byId("leg-ledger").innerHTML = state.legs
    .map((item, index) => {
      const result = metrics.selected.legs[index];
      if (!item.enabled)
        return `<tr class="inactive"><td>Leg ${index + 1}</td><td>Inactive</td><td>—</td><td>—</td><td>—</td><td>—</td><td>—</td><td>Not evaluated</td></tr>`;
      const pricing = result.pricing!;
      const contract = `${item.side === "long" ? "Long" : "Short"} ${item.quantity} ${item.type}`;
      const barrier =
        item.barrierType === "none"
          ? "Vanilla"
          : `${item.barrierType} @ ${item.barrier.toFixed(0)}`;
      const stateText =
        item.barrierType === "none"
          ? "Always active"
          : `${result.hit ? "Touched" : "Not touched"} · ${result.active ? "active" : "inactive"}`;
  ...
  byId("leg-totals").innerHTML =
    `<tr><th colspan="4">Strategy total</th><td>${metrics.netPremium.toFixed(2)}</td><td>${metrics.selected.payoff.toFixed(2)}</td><td>${signed(metrics.selected.pnl, 2)}</td><td>${activeCount} active leg${activeCount === 1 ? "" : "s"}</td></tr>`;
}

function renderBarrierStates(state: StrategyState, metrics: StrategyMetrics): void {
  const minimum = state.spot * 0.4,
    maximum = state.spot * 1.6;
  const position = (value: number) =>
    Math.max(0, Math.min(100, ((value - minimum) / (maximum - minimum)) * 100));
  const pathLow = Math.min(state.observedLow, state.terminal, state.spot),
    pathHigh = Math.max(state.observedHigh, state.terminal, state.spot);
  byId("barrier-states").innerHTML = state.legs
    .map((item, index) => {
      const result = metrics.selected.legs[index];
      if (!item.enabled)
        return `<article class="barrier-state inactive"><span>Leg ${index + 1}</span><strong>Inactive leg</strong><p>No contract or barrier event is evaluated.</p></article>`;
      if (item.barrierType === "none")
    ...
    .join("");
  const barrierCount = state.legs.filter(
    (item) => item.enabled && item.barrierType !== "none",
  ).length;
  byId("barrier-note").textContent = barrierCount
    ? "The displayed premium and before-expiry risk scale vanilla value by an estimated touch or survival probability. That is intentionally transparent and fast, but it is not a barrier-option valuation formula."
    : "All active legs are vanilla, so the observed path range does not change their expiry payoff.";
}

function renderReferenceTables(state: StrategyState): void {
  byId("strategy-catalogue").innerHTML = catalogue
    .map(
      (row) =>
        `<tr class="${state.presetId === row[0] ? "selected-row" : ""}"><td>${esc(row[1])}</td><td>${esc(row[2])}</td><td>${esc(row[3])}</td><td>${esc(row[4])}</td></tr>`,
    )
    .join("");
  byId("strategy-rules").innerHTML = rules
