...
function generatePath({ seed = 1, tenor = 3, vol = 30, scenario = "random" }) {
  ...
  const anchors = scenarioAnchors[scenario] || scenarioAnchors.decline;
  let noise = 0;
  for (let day = 1; day <= end; day++) {
...
function breachInfo(path, barrier) {
...
function evaluateRC(path, p) {
  ...
  for (let index = 0; index < obs.length; index++) {
    ...
    if (hasBarrier && p.barrierObservation === "maturity" && maturity && level <= p.barrier)
      barrierBreached = true;
    coupons += periodCoupon;
    let decision = "Continue";
    if (!maturity && index >= 1 && variant === "autocall" && level >= p.callLevel) {
      called = true;
    ...
    if (!maturity && index >= 1 && variant === "issuer") {
      const exercise =
        p.callPolicy === "first" || (p.callPolicy === "above" && level >= p.callLevel);
      if (exercise) {
        called = true;
...
function evaluateLock(path, p) {
  ...
  for (const day of obs) {
    ...
    const coupon = p.style === "par" ? (p.coupon * (day - previous)) / DAYS : 0;
    coupons += coupon;
    events.push({
      day,
      level,
  ...
  if (p.style === "par") {
    barrierBreached = final < p.barrier;
...
function evaluate(mode, path, p) {
  return mode === "rc"
    ? evaluateRC(path, p)
    : mode === "coupon"
      ? evaluateCoupon(path, p)
      : evaluateLock(path, p);
}
