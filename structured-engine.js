...
(function(root){
  ...
  function observations(tenor,frequency){
    const end=Math.round(tenor*DAYS),count=Math.max(1,Math.round(tenor*frequency)),days=[];
    for(let i=1;i<=count;i++)days.push(Math.min(end,Math.round(i*end/count)));
    return [...new Set(days)];
  }
  ...
  root.StructuredEngine={DAYS,seeded,observations,generatePath,evaluate,simulate};
