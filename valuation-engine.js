(function (root) {
  ...
  function designNote(terms) {
    ...
    const optionUnit = bsCall(terms.initialSpot, terms.strike, terms.tenor, terms.rate, terms.dividend, terms.volatility) * Math.exp(-terms.issuerSpread * terms.tenor);
  ...
  function markNote(note, market) {
    ...
    if (remaining === 0) {
      return { remaining, bond: note.nominal, option: maturityPayoff - note.nominal, modelValue: maturityPayoff, bid: maturityPayoff, maturityPayoff };
    }
    ...
    const option = note.participation * bsCall(market.spot, note.strike, remaining, market.rate, market.dividend, market.volatility) * creditFactor;
  ...
  function buildAttribution(note, market) {
    const issueMarket = {
      elapsed: 0,
    ...
    steps.push({ label: "Exit deduction", from: previous, to: Math.max(0, previous - market.exitCost), change: -Math.min(previous, market.exitCost) });
    return { start: note.estimatedValue, end: Math.max(0, previous - market.exitCost), steps };
  }

  function valueCurve(note, market, minimum, maximum, count) {
    const points = [];
    ...
    for (let index = 0; index < n; index += 1) {
      ...
      points.push({ spot, modelValue: marked.modelValue, bid: marked.bid, maturityPayoff: marked.maturityPayoff });
    }
    return points;
  }

  const api = { normCdf, bsCall, designNote, markNote, buildAttribution, valueCurve };
